...
namespace ERP.DAL
{
	...
	public partial class Agent
	{
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(ERP.Model.Agent model)
		{
            if (model.Ident == null)
            {
                model.Ident = StaticDbMethod.GetNextAutoIdent(typeof(ERP.Model.Agent).Name, SearchStr.PrefixOfAgent);
            }
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Agent(");
			strSql.Append("Ident,Address,Name,Description,ModifyUser,AgentLevel)");
			strSql.Append(" values (");
			strSql.Append("@Ident,@Address,@Name,@Description,@ModifyUser,@AgentLevel)");
			SqlParameter[] parameters = {
					new SqlParameter("@Ident", SqlDbType.Char,15),
					new SqlParameter("@Address", SqlDbType.Char,40),
					...
					new SqlParameter("@AgentLevel", SqlDbType.TinyInt,1)};
			...
			parameters[5].Value = model.AgentLevel;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(ERP.Model.Agent model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public ERP.Model.Agent GetModel(long OID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 OID,Ident,Address,Name,Description,ModifyUser,AgentLevel from Agent ");
			strSql.Append(" where OID=@OID ");
			SqlParameter[] parameters = {
					new SqlParameter("@OID", SqlDbType.BigInt,8)			};
			parameters[0].Value = OID;
		...
		/// 得到一个对象实体
		/// </summary>
		public ERP.Model.Agent DataRowToModel(DataRow row)
		{
			ERP.Model.Agent model=new ERP.Model.Agent();
			if (row != null)
			{
				...
				if(row["AgentLevel"]!=null && row["AgentLevel"].ToString()!="")
				{
                    model.AgentLevel = (ERP.Model.AgentLevelEnum)Enum.Parse(typeof(ERP.Model.AgentLevelEnum), row["AgentLevel"].ToString());
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select OID,Ident,Address,Name,Description,ModifyUser,AgentLevel ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" OID,Ident,Address,Name,Description,ModifyUser,AgentLevel ");
			strSql.Append(" FROM Agent ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        /// 得到一个对象实体
        /// </summary>
        public ERP.Model.Agent GetModel(string Ident)
        {
