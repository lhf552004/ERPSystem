...
namespace ERP
{
    public partial class ExpectReceiptAccountWinForm : Form
    {
        public ExpectReceiptAccountWinForm()
        {
            InitializeComponent();
        }
        private  BLL.Product bllProduct= new BLL.Product();
        private  BLL.Warehouse bllWarehouse = new BLL.Warehouse();
        private BLL.ExpectReceiptAccount bllExpectReceiptAccount = new BLL.ExpectReceiptAccount();
        private  Model.Warehouse curWarehouse =null;
        private ERP.Model.ExpectReceiptAccount _curExpectReceiptAccount = null;
        ...
        private void _loadData()
        {
            try
            {
                ds = bllExpectReceiptAccount.GetAllList();
                if (ds != null)
                {

                    ExpectReceiptAccountGridView.DataSource = ds.Tables[0];
                    ExpectReceiptAccountGridView.Columns["Dis_OID"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["Operator"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["Description"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["ModifyUser"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["Description"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["Auditor"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["CreationDate"].Visible = false;
                    ExpectReceiptAccountGridView.Columns["Loaned"].Visible = false;
        ...
        private void _updateControlText(int theIndex)
        {
            try
            {
                ...
                int OID = int.Parse(ExpectReceiptAccountGridView.Rows[theIndex].Cells[0].Value.ToString());
                //curProduct = bllProduct.GetModel(OID);
                _curExpectReceiptAccount = bllExpectReceiptAccount.GetModel(OID);
                if (_curExpectReceiptAccount != null)
                {
                    ProdIdent.Text = _curExpectReceiptAccount.Ident;
                    //ProdNum.Text = _curExpectReceiptAccount.Number.ToString();
                    //ProdCategory.Text = Enum.GetName(typeof(ProductCategory), curProduct.Category);
                    //ProdPrice.Text = curProduct.Price.ToString();
                    ProdDescription.Text = _curExpectReceiptAccount.Description;
                    ModifyUser.Text = _curExpectReceiptAccount.ModifyUser;
        ...
        {
            _curExpectReceiptAccount.Description = ProdDescription.Text;
        }
        ...
        private void ProStatisticsGridView_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Up || e.KeyCode == Keys.Down)
            {
                currentIndex = ExpectReceiptAccountGridView.CurrentRow.Index;
                _updateControlText(currentIndex);
            }
        }
